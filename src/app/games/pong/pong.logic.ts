...
export class Pong {
  ...
  constructor(e: HTMLCanvasElement) {
    ...
    this.paddleComp.x = this.canvas.width - this.paddleWidth - this.paddleUser.x;
  ...
  writePauseTimeRemaining(timeRemaining: number) {
    ...
    // clear background
    canvas.drawFilledRect(width / 2 - 7, height / 2 - 125, 30, 30, backgroundColor);
    // write text
    canvas.writeText(
      `${timeRemaining}`,
      width / 2, height / 2 - 100,
      30, this.homeScreenTextColor);
  }
  ...
  onHover(event: MouseEvent) {
    if (this.canvas) {
      ...
      this.paddleUser.y = event.clientY - canvas.top -
        root.scrollTop - this.paddleHeight / 2;
  ...
  drawBackground() {
    ...
    this.canvas.drawOutlineRect
      (offset, offset, width - 2 * offset, height - 2 * offset, outlineColor);
  }
  ...
  renderStartScreen() {
    ...
    const sBY = 200;  // scoreBoardY

    const {
      homeScreenTextColor, userScoreColor, compScoreColor,
      playerScore, computerScore
    } = this;

    this.canvas.writeText(t1, 70, sBY - 50, 25, homeScreenTextColor);
    ...
    if (playerScore !== computerScore) {
      ...
      this.canvas.writeText(winnerText, sBX - 50, sBY, 30, winnerTextColor);
      this.canvas.writeText(t3, sBX, sBY + 50, 20, userScoreColor);
      this.canvas.writeText(t4, sBX + 22, sBY + 80, 20, compScoreColor);
    }
  }
  ...
  handlePaddleCollision() {
    const {
      ball, paddleUser, paddleComp,
      paddleHeight, paddleWidth, offset, canvas,
  ...
  drawPaddles() {
    ...
    }
    {
      ...
      this.canvas.drawLine(offset, offset, width - offset, offset, 1, paddleColor);
      this.canvas.drawLine(
        offset, height - offset, width - offset, height - offset, 1, paddleColor
      );
    }
  }
  ...
  writeScore() {
    ...
    this.canvas.writeText(t2, 500, 15, 15, this.homeScreenTextColor);
    this.canvas.writeText(t3, 270, 15, 15, this.homeScreenTextColor);
  }
}
